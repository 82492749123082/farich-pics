...
class BoardsGenerator:
    ...
    def AddROOT(self, *rootFiles):
        for rootFile in rootFiles:
            ...
            event = np.concatenate(
                [group.values for name, group in pg.progressbar(grouped)]
            )
    ...
    def GenerateBoards(
        ...
        if self.__boards_sizes is None:
            ...
            if (
                self.__boards_sizes
                is not (size[0], size[1], int(10 ** 9 / freq / ticks))
                or self.__freq is not freq
            ):
                raise Exception(
                    "self.__boards_sizes and self.__freq should match previous values"
                )
    ...
    def __add_to_board(self, board, arr, noise_level, augmentations):
        ...
        board = np.concatenate([board, arr]) if board.size != 0 else arr
        return board

    def SaveBoards(self, filepath):
        pass
